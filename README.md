...
Alternatively, if you want to create the Conda environment manually, install the core compiled/geospatial stack first with Python 3.13 or newer:

```bash
git clone git@github.com:ehavazli/snowsar.git
cd snowsar
conda create -n snowsar -c conda-forge "python>=3.13" rasterio gdal sardem shapely geopandas contextily leafmap folium matplotlib mintpy libgdal-hdf5 h5py earthaccess scipy jupyterlab nb_conda_kernels pytest pip
conda activate snowsar
```
