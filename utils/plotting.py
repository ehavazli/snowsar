    ...
    *,
    ...
    for site_name, df in results.items():
        ...
        if x_axis == "days_since_reference":
            ...
            raise ValueError("x_axis must be 'days_since_reference' or 'date'")
    ...
    if x_axis == "days_since_reference":
        ...
        for d in dates:
            axes[0].axvline(d, color="k", linestyle="--", alpha=0.4, linewidth=1)
    ...
    all_dates = np.array(pd.to_datetime(all_dates).normalize(), dtype="datetime64[D]")
    ...
    std_deltas = np.array([np.nanstd(all_deltas[all_dates == d]) for d in unique_dates])
